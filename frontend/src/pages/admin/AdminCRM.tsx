...
function filterAppeals<T extends { topic: string; main_text: string; tag?: string | null }>(
  ...
  if (q) {
    out = out.filter((a) => a.topic.toLowerCase().includes(q) || a.main_text.toLowerCase().includes(q))
  }
  if (tagFilter) {
    out = out.filter((a) => (a.tag || '') === tagFilter)
  }
  return out
}

export default function AdminCRM() {
  const { appeals, loading, updatingId, setSelectedAppeal, updateStatus } = useAdmin()
  const [searchQuery, setSearchQuery] = useState('')
  const [tagFilter, setTagFilter] = useState('')
  const filtered = useMemo(() => filterAppeals(appeals, searchQuery, tagFilter), [appeals, searchQuery, tagFilter])

  return (
    <section className="admin-section card">
      <h2 className="admin-page-title">CRM — все обращения</h2>
